...
foreach($data as $x => $x_value) {
    ...
    $sql = "SELECT id FROM bookorder WHERE subject = '$x' AND price = 200 AND (state = '已收到書' or state = '尚未收到書')";
     $result = $conn->query($sql);
    $data[$x][0] = $result->num_rows;



     $sql = "SELECT id FROM bookorder WHERE subject = '$x' AND price = 300 AND (state = '已收到書' or state = '尚未收到書')";
     $result = $conn->query($sql);
    $data[$x][1] = $result->num_rows;


     $sql = "SELECT id FROM bookorder WHERE subject = '$x' AND price = 500 AND (state = '已收到書' or state = '尚未收到書')";
     $result = $conn->query($sql);
   $data[$x][2] = $result->num_rows;


     $sql = "SELECT id FROM bookorder WHERE subject = '$x' AND price = 700 AND (state = '已收到書' or state = '尚未收到書')";;
     $result = $conn->query($sql);
