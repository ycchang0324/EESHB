...
const OrderStatusBar = (props) => {
  ...
  const handleStateChange = (blank, newState) => {
    ...
    axios
      .post("https://book.ntuee.org/backend/manage/changeState.php", {
        id: props.data.id,
        state: newState,
      })
      .then((data) => {
  ...
  const handleBuyerIdChange = (e) => {
    if (e.keyCode === 13) {
      alert(buyerId);
      axios
        .post("https://book.ntuee.org/backend/manage/changeBuyerId.php", {
          id: props.data.id,
          buyerId: buyerId,
        })
        .then((data) => {
  ...
  return (
    <tr id={"OrderStatusBar_container_" + props.index}>
      ...
      <td
        id={"SellerStateBar_" + props.index + "_background"}
        style={{ boxSizing: "padding-box" }}
      >
        <SellerStateBar
          id={"SellerStateBar_" + props.index}
